...
--// Services
local TweenService = AstralLib.GetService("TweenService")
local RunService = AstralLib.GetService("RunService")
local HttpService = AstralLib.GetService("HttpService")
local ContentProvider = AstralLib.GetService("ContentProvider")
local UserInputService = AstralLib.GetService("UserInputService")
local Lighting = AstralLib.GetService("Lighting")
local Players = AstralLib.GetService("Players")
...
local function GetGui()
	...
	local parent = RunService:IsStudio() 
		...
		or (cloneref and cloneref(AstralLib.GetService("CoreGui")) or AstralLib.GetService("CoreGui"))
...
--// Library Functions
function AstralLib:Window(Settings)
		...
		function SectionFunctions:Tab(Settings)
			...
			function TabFunctions:Section(Settings)
				...
				function SectionFunctions:Button(Settings, Flag)
					...
					if Flag then
						AstralLib.Options[Flag] = ButtonFunctions
					end
				...
				function SectionFunctions:Toggle(Settings, Flag)
					...
					if Flag then
						AstralLib.Options[Flag] = ToggleFunctions
					end
				...
				function SectionFunctions:Slider(Settings, Flag)
					...
					if Flag then
						AstralLib.Options[Flag] = SliderFunctions
					end
				...
				function SectionFunctions:Input(Settings, Flag)
					...
					if Flag then
						AstralLib.Options[Flag] = InputFunctions
					end
				...
				function SectionFunctions:Keybind(Settings, Flag)
					...
					if Flag then
						AstralLib.Options[Flag] = KeybindFunctions
					end
				...
				function SectionFunctions:Dropdown(Settings, Flag)
					...
					if Flag then
						AstralLib.Options[Flag] = DropdownFunctions
					end
				...
				function SectionFunctions:Colorpicker(Settings, Flag)
					...
					if Flag then
						AstralLib.Options[Flag] = ColorpickerFunctions
					end
				...
				function SectionFunctions:Header(Settings, Flag)
					...
					if Flag then
						AstralLib.Options[Flag] = HeaderFunctions
					end
				...
				function SectionFunctions:Label(Settings, Flag)
					...
					if Flag then
						AstralLib.Options[Flag] = LabelFunctions
					end
				...
				function SectionFunctions:SubLabel(Settings, Flag)
					...
					if Flag then
						AstralLib.Options[Flag] = SubLabelFunctions
					end
				...
				function SectionFunctions:Paragraph(Settings, Flag)
					...
					if Flag then
						AstralLib.Options[Flag] = ParagraphFunctions
					end
			...
			function TabFunctions:InsertConfigSection(Side)
				...
				local configSelection = configSection:Dropdown({
					...
					Multi = false,
					Required = false,
					Options = AstralLib:RefreshConfigList(),
				...
				configSection:Button({
					...
					Callback = function()
						...
						local success, returned = AstralLib:SaveConfig(inputPath)
						if not success then
							WindowFunctions:Notify({
								Title = "Interface",
						...
						configSelection:InsertOptions(AstralLib:RefreshConfigList())
					end,
				})
				...
				configSection:Button({
					Name = "Load Config",
					Callback = function()
						local success, returned = AstralLib:LoadConfig(configSelection.Value)
						if not success then
							WindowFunctions:Notify({
								Title = "Interface",
				...
				configSection:Button({
					...
					Callback = function()
						local success, returned = AstralLib:SaveConfig(configSelection.Value)
						if not success then
							WindowFunctions:Notify({
								Title = "Interface",
				...
				configSection:Button({
					...
					Callback = function()
						...
						writefile(AstralLib.Folder .. "/settings/autoload.txt", name)
				...
				if isfile(AstralLib.Folder .. "/settings/autoload.txt") then
					local name = readfile(AstralLib.Folder .. "/settings/autoload.txt")
					autoloadLabel:UpdateName("Autoload config: " .. name)
				end
			end
	...
	local ClassParser = {
		["Toggle"] = {
			...
			Load = function(Flag, data)
				if AstralLib.Options[Flag] and data.state then
					AstralLib.Options[Flag]:UpdateState(data.state)
				end
			end
		},
		["Slider"] = {
			...
			Load = function(Flag, data)
				if AstralLib.Options[Flag] and data.value then
					AstralLib.Options[Flag]:UpdateValue(data.value)
				end
			end
		},
		["Input"] = {
			...
			Load = function(Flag, data)
				if AstralLib.Options[Flag] and data.text and type(data.text) == "string" then
					AstralLib.Options[Flag]:UpdateText(data.text)
				end
			end
		},
		["Keybind"] = {
			...
			Load = function(Flag, data)
				if AstralLib.Options[Flag] and data.bind then
					AstralLib.Options[Flag]:Bind(Enum.KeyCode[data.bind])
				end
			end
		},
		["Dropdown"] = {
			...
			Load = function(Flag, data)
				if AstralLib.Options[Flag] and data.value then
					AstralLib.Options[Flag]:UpdateSelection(data.value)
				end
			end
		},
		["Colorpicker"] = {
			...
			Load = function(Flag, data)
				...
				if AstralLib.Options[Flag] and data.color then
					AstralLib.Options[Flag]:SetColor(HexToColor3(data.color)) 
					if data.alpha then
						AstralLib.Options[Flag]:SetAlpha(data.alpha)
					end
				end
			end
		}
	...
	function AstralLib:SetFolder(Folder)
		if isStudio then return "Config system unavailable." end

		AstralLib.Folder = Folder;
		BuildFolderTree()
	end

	function AstralLib:SaveConfig(Path)
		...
		local fullPath = AstralLib.Folder .. "/settings/" .. Path .. ".json"

		local data = {
			objects = {}
		}

		for flag, option in next, AstralLib.Options do
	...
	function AstralLib:LoadConfig(Path)
		if isStudio or not (isfile and readfile) then return "Config system unavailable." end
		...
		local file = AstralLib.Folder .. "/settings/" .. Path .. ".json"
	...
	function AstralLib:RefreshConfigList()
		if isStudio or not (isfolder and listfiles) then return "Config system unavailable." end

		local list = (isfolder(AstralLib.Folder) and isfolder(AstralLib.Folder .. "/settings")) and listfiles(AstralLib.Folder .. "/settings") or {}
...
function AstralLib:Demo()
	local Window = AstralLib:Window({
		Title = "AstralLib Demo",
